...
let 订单表 = computed(() => {
  ...
  if (全局状态.订单状态分类 === '未定片') {
    要显示的订单 = 库.订单表.filter((行: any) => {
      return 行.订单进度 !== '已完成' && (!行.左镜片订货日 || !行.右镜片订货日) && 行.镜片 !== '' && !行.镜片.includes('蔡司')
  ...
  if (全局状态.订单状态分类 === '直接加工') {
    要显示的订单 = 库.订单表.filter((行: any) => {
      return 行.镜框选项 === '直接加工' && 行.订单进度 !== '已完成'
    })
  }
  if (全局状态.订单状态分类 === '只买镜框') {
    要显示的订单 = 库.订单表.filter((行: any) => {
      return 行.镜框选项 === '只买镜框' && 行.订单进度 !== '已完成'
    })
  }
...
let 已完成订单数量 = computed(() => 库.订单表.filter((行: any) => {
  return 行.订单进度 === "已完成"
}).length)
let 未完成订单数量 = computed(() => 库.订单表.filter((行: any) => {
  return 行.订单进度 !== "已完成"
}).length)
let 未订片订单数量 = computed(() => 库.订单表.filter((行: any) => {
  return 行.订单进度 !== '已完成' && (!行.左镜片订货日 || !行.右镜片订货日) && 行.镜片 !== '' && !行.镜片.includes('蔡司')
}).length)
let 有镜片未完成订单数量 = computed(() => 库.订单表.filter((行: any) => {
  return 行.订单进度 !== '已完成' && 行.镜片 !== ''
}).length)
let 未发镜框订单数量 = computed(() => 库.订单表.filter((行: any) => {
  return 行.订单进度 !== '已完成' && (行.选定镜框 || 行.试戴镜框.filter((item) => item !== null && item !== "").length !== 0)
      && !行.镜框发货日 && 行.镜框选项 !== '直接加工'
}).length)
let 直接加工订单数量 = computed(() => 库.订单表.filter((行: any) => {
  return 行.镜框选项 === '直接加工' && 行.订单进度 !== '已完成'
}).length)

let 只买镜框订单数量 = computed(() => 库.订单表.filter((行: any) => {
  return 行.镜框选项 === '只买镜框' && 行.订单进度 !== '已完成'
}).length)
...
<template>
  <div class="整页 ">
    ...
    <div class="顶部 ">
      ...
      <div @click="全局状态.订单状态分类 = '已完成'; 库.当前页 = 1"
           :class="{ 按钮: true, 选中按钮: 全局状态.订单状态分类 == '已完成' }" class="按钮">
        已完成{{ 已完成订单数量 }}
      </div>


      <div class="按钮组">
        <div @click="全局状态.订单状态分类 = '未完成'; 库.当前页 = 1"
             :class="{ 选中按钮: 全局状态.订单状态分类 == '未完成' }" class="按钮">
          未完成{{ 未完成订单数量 }}
        </div>
        <div @click="全局状态.订单状态分类 = '有镜片未完成'; 库.当前页 = 1"
             :class="{ 选中按钮: 全局状态.订单状态分类 == '有镜片未完成' }" class="按钮">
          有镜片{{ 有镜片未完成订单数量 }}
        </div>
        <div @click="全局状态.订单状态分类 = '蔡司镜片'; 库.当前页 = 1"
      ...
      <div class="按钮组">
        <div @click="全局状态.订单状态分类 = '未定片'; 库.当前页 = 1"
             :class="{ 选中按钮: 全局状态.订单状态分类 == '未定片' }" class="按钮">
          未定片{{ 未订片订单数量 }}
        </div>
        <div @click="全局状态.订单状态分类 = '直接加工'; 库.当前页 = 1"
             :class="{ 选中按钮: 全局状态.订单状态分类 == '直接加工' }" class="按钮">
          直接加工{{ 直接加工订单数量 }}
        </div>
        <div @click="全局状态.订单状态分类 = '只买镜框'; 库.当前页 = 1"
             :class="{ 选中按钮: 全局状态.订单状态分类 == '只买镜框' }" class="按钮">
          只买镜框{{ 只买镜框订单数量 }}
        </div>
        <div @click="全局状态.订单状态分类 = '未发镜框'; 库.当前页 = 1"
             :class="{ 选中按钮: 全局状态.订单状态分类 == '未发镜框' }" class="按钮">
          未发镜框{{ 未发镜框订单数量 }}
        </div>
...
.整页 {
  ...
  .顶部 {
    gap: 10px;
    height: 35px;
    grid-template-columns: 80px 120px 400px 400px  1fr 200px 150px;
