...
let 利润计算属性 = computed(() => {
    ...
    for (let i = 1; i <= 12; i++) {
        ...
        for (let j = 1; j <= 31; j++) {
            ...
            每日利润[每月][日期] = { 镜框利润: 0, 镜片利润: 0, 总利润: 0, 镜片订单数量: 0, 奖金: 0, }
        }
            ...
            if (库.订单表[i].总利润) {
                ...
                if (库.订单表[i].镜片下单日) {
                    ...
                    if (每日利润[镜片月][镜片日]) {
                        ...
                        if (每日利润[镜片月][镜片日].镜片订单数量 >= 3) { 每日利润[镜片月][镜片日].奖金 = 10 }
                        if (每日利润[镜片月][镜片日].镜片订单数量 >= 5) { 每日利润[镜片月][镜片日].奖金 = 30 }
                        if (每日利润[镜片月][镜片日].镜片订单数量 >= 7) { 每日利润[镜片月][镜片日].奖金 = 50 }
                        if (每日利润[镜片月][镜片日].镜片订单数量 >= 9) { 每日利润[镜片月][镜片日].奖金 = 100 }
                        if (每日利润[镜片月][镜片日].镜片订单数量 >= 11) { 每日利润[镜片月][镜片日].奖金 = 150 }
                        if (每日利润[镜片月][镜片日].镜片订单数量 >= 13) { 每日利润[镜片月][镜片日].奖金 = 200 }
                        if (每日利润[镜片月][镜片日].镜片订单数量 >= 15) { 每日利润[镜片月][镜片日].奖金 = 260 }
                        if (每日利润[镜片月][镜片日].镜片订单数量 >= 17) { 每日利润[镜片月][镜片日].奖金 = 320 }
                        if (每日利润[镜片月][镜片日].镜片订单数量 >= 20) { 每日利润[镜片月][镜片日].奖金 = 400 }
                        每月利润[镜片月 + '月总计'].利润 += 库.订单表[i].总利润
                        每月利润[镜片月 + '月总计'].订单数量 += 1
                    }
