    ...
    for (let i in 库.订单表) {
      if (库.订单表[i].总利润) {
        ...
        if (库.订单表[i].镜片下单日) {
          ...
          if (每日利润[镜片月][镜片日]) {
            每日利润[镜片月][镜片日].总利润 =  每日利润[镜片月][镜片日].总利润 + 库.订单表[i].镜片利润 - 库.订单表[i].优惠
            每日利润[镜片月][镜片日].总利润 = parseFloat(每日利润[镜片月][镜片日].总利润.toFixed(2))

            每日利润[镜片月][镜片日].镜片利润 = 每日利润[镜片月][镜片日].镜片利润 + 库.订单表[i].镜片利润 - 库.订单表[i].优惠
            每日利润[镜片月][镜片日].镜片利润 = parseFloat(每日利润[镜片月][镜片日].镜片利润.toFixed(2))

            每月利润[镜片月 + '月总计'].总利润 =  每月利润[镜片月 + '月总计'].总利润+ 库.订单表[i].镜片利润 - 库.订单表[i].优惠
            每月利润[镜片月 + '月总计'].镜片利润 = 每月利润[镜片月 + '月总计'].镜片利润 + 库.订单表[i].镜片利润 - 库.订单表[i].优惠
